...
class Student(models.Model):
    ...
    def lessons(self, paid: bool = None) -> QuerySet:
        q: QuerySet = QuerySet.none()
        match paid:
            case None:
                for lessonplan in self.lessonplan.all():
                    q = q.union(lessonplan.lessons.all())
            case _:
                for lessonplan in self.lessonplan.all():
    ...
    def pay(self, value: int):
        # Get the oldest unpaid lesson and keep paying off until the payment runs out
        # TBC
        pass 
...
class Lesson(Event):
    ...
    def clean(self):
        ...
        if conflicting_lessons.exists() and self.lessonplan != conflicting_lessons.first().lessonplan:
